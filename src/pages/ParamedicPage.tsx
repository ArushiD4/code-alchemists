...
export default function ParamedicPage() {
    ...
    const validateNormalForm = (): boolean => {
        ...
        if (!vitals.acuity) {
            toast.error('Acuity Level is required');
            return false;
        }
    ...
    const submitStandardReport = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setVitals({ name: '', hr: '', bp: '', spo2: '', acuity: '', notes: '' }); // Clear form
        } catch (error) {
            ...
            <div className="p-4 flex-1 flex flex-col w-full h-full max-w-lg mx-auto relative">
                        ...
                        {selectedColor && (
                            <div className="flex-1 flex flex-col gap-4 pb-4 w-full h-full relative">
                                <div className="absolute top-2 left-2 z-10 px-4 py-1.5 rounded-full text-xs font-bold tracking-widest uppercase"
                                    style={{
                                        backgroundColor: selectedColor === 'RED' ? '#dc2626' :
                ...
                {!isMciMode && (
                    <form onSubmit={submitStandardReport} className="flex-1 flex flex-col gap-6 pb-8 overflow-y-auto">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="col-span-2 mt-2">
                                    ...
                                    <input
                                        ...
                                        required
                                        pattern="^\d{2,3}\/\d{2,3}$"
                                        title="Format must be Systolic/Diastolic, e.g., 120/80"
                        ...
                        <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm">
                            <label className="block text-xs font-bold tracking-widest text-slate-500 uppercase mb-3 text-red-500">Acuity Level *</label>
                            <div className="relative">
                                <select
                                    value={vitals.acuity}
                                    onChange={(e) => setVitals({ ...vitals, acuity: e.target.value })}
                                    required
                                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-4 text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all text-lg font-bold appearance-none"
                                >
                                    <option value="" disabled>Select Acuity...</option>
